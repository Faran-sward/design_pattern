...
import SpaceGreenhouse.common.product.crops.*;
import SpaceGreenhouse.common.product.fertilizer.AdvancedFertilizer;
import SpaceGreenhouse.common.product.fertilizer.MiddleFertilizer;
...
public class SpaceGreenhouseWarehouse implements ISpaceGreenhouseWareHouse {
    ...
    private SpaceGreenhouseWarehouse(SEER SEER) {
        this.SEER=SEER;
    ...
    public static SpaceGreenhouseWarehouse getInstance(SEER SEER){
        return new SpaceGreenhouseWarehouse(SEER);
    }
    ...
    public <T extends IProduct> boolean buyObject(T object, int num, String methodName) {
        ...
        //需要有一个赛尔角色类，判断剩余赛尔豆是否大于等于交换金额，是则返回true，并扣除相应大小的赛尔豆
        //调用适配器
        ...
        try {
            Method method = SpaceGreenhouseWarehouse.class.getDeclaredMethod(methodName);
    ...
    public boolean sellCrops(AbstractCrops crops, int num) {
        ...
        System.out.println("卖出成功，共进账" + crops.getPrice() * num + "赛尔豆！" + "当前余额(" + SEER.getMoney() + ")");
        return true;
    }
